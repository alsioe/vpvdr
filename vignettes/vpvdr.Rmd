...
# Get vpvdr from github using devtools - note there are large files so we need
# to first change the timeout
# 
# If you are updating vpvdr you might need to detach first
# detach('package:vpvdr')
...
df_subj <- summarise_correct_by_subj(list = simulated_rats,
                                     groups = c('slow', 'medium', 'fast'))
...
# Let's try that again with different alpha-loss instead
simulated_rats <- simulate_choices_vpvd(nTrials = nTrials,
                                       nIter = nIter,
                                       nGroups = nGroups,
                                       alpha_w = rep(0.002, nIter * nGroups),
                                       alpha_l = c(rep(0.002, nIter),
                                                   rep(0.004, nIter),
                                                   rep(0.006, nIter)
                                                   ),
                                       beta = rep(2, nIter * nGroups),
...
{ # Current workaround chunk
if (six_parameters[1] %in% names(group_params)) {
    w <- group_params$reward_rate_by_group_drug[draws, 1:nGroups, 1]
} else {
    w <- matrix(data = rep(0, nIter*nGroups),
                nrow = nIter,
                ncol = nGroups)
}

if (six_parameters[2] %in% names(group_params)) {
    l <- group_params$punish_rate_by_group_drug[draws, 1:nGroups, 1]
} else {
    l <- matrix(data = rep(0, nIter*nGroups),
                nrow = nIter,
                ncol = nGroups)
}

if (six_parameters[3] %in% names(group_params)) {
    u <- group_params$prob_discount_by_group_drug[draws, 1:nGroups, 1]
} else {
    u <- matrix(data = rep(0, nIter*nGroups),
                nrow = nIter,
                ncol = nGroups)
}

if (six_parameters[4] %in% names(group_params)) {
    b <- group_params$reinf_sensitivity_by_group_drug[draws, 1:nGroups, 1]
} else {
    b <- matrix(data = rep(0, nIter*nGroups),
                nrow = nIter,
                ncol = nGroups)
}

if (six_parameters[5] %in% names(group_params)) {
    k <- group_params$side_stickiness_by_group_drug[draws, 1:nGroups, 1]
} else {
    k <- matrix(data = rep(0, nIter*nGroups),
                nrow = nIter,
                ncol = nGroups)
}

if (six_parameters[6] %in% names(group_params)) {
    t <- group_params$stimulus_stickiness_by_group_drug[draws, 1:nGroups, 1]
} else {
    t <- matrix(data = rep(0, nIter*nGroups),
                nrow = nIter,
                ncol = nGroups)
}
} #end of workaround chunk
...
ppc_subj <- summarise_correct_by_subj(list = post_pred_check, 
                                      groups = c('veh', 'low', 'high'))
